...
<script>
    var table_header = {
        "donor": {
            ...
            "header": ['ID', 'Nama', 'Status', ' Gol. Darah', 'Tanggal Donor', 'Tanggal Request']
        }
    };
    ...
    function fill_table(data, table, page, rebuild_distinct = false) {
        ...
        if (rebuild_distinct) {
            //empty all select
            $("#distinct-blood_type").empty();
            $("#distinct-status").empty();
            $("#distinct-year_month").empty();
            //add none option
            $("#distinct-blood_type").append('<option value="">Semua</option>');
            $("#distinct-status").append('<option value="">Semua</option>');
            $("#distinct-year_month").append('<option value="">Semua</option>');
            //fill blood type select
            for (var i = 0; i < distinct_data["blood_type"].length; i++) {
                $("#distinct-blood_type").append('<option value="' + distinct_data["blood_type"][i] + '">' + distinct_data["blood_type"][i] + '</option>');
            }
            //fill status select
            for (var i = 0; i < distinct_data["status"].length; i++) {
                $("#distinct-status").append('<option value="' + distinct_data["status"][i] + '">' + distinct_data["status"][i] + '</option>');
            }

            //fill year_month select
            for (var i = 0; i < distinct_data["year_month"].length; i++) {
                $("#distinct-year_month").append('<option value="' + distinct_data["year_month"][i] + '">' + distinct_data["year_month"][i] + '</option>');
            }
        }
    }
    ...
    $("#btn-search").click(function () {
        ...
        if (Object.keys(distincts).length == 0) {
            ...
            //fill table
            fill_table(new_data, current_table, 0, false);
        }
